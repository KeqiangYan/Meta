...
env<-read.csv("t2.txt",header = T,sep = "\t")
...
sc2<-sc2[,c(-2,-7,-9,-10,-19,-20,-21,-24,-26,-27)]
...
e31<-e2[,c(1,4)]
#将两个变量单独拿出来组成新的数据框进行分析
sc.rda4<-rda(sc3~Temperature+Chla,data=e31)
ef=envfit(sc.rda4,e31,permu=999)
ef
RsquareAdj(sc.rda4)
anova(sc.rda4)

#三种环境因子的分析
sc.rda3<-rda(sc3~Temperature+Salinity+Chla+PH,data=e3)
permutest(sc.rda3,permu=999)
e31<-e2[,c(1,2,3,4)]
...
envi
line_x =c(0,envi[1,2],0,envi[2,2],0,envi[3,2],0,envi[4,2])
line_x
line_y =c(0,envi[1,3],0,envi[2,3],0,envi[3,3],0,envi[4,2])
line_y
line_g =c("Temperature","Temperature","Salinity","Salinity","Chla","Chla","PH","PH")
line_g
